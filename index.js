...
require('./lib/learning.js')
nocache('./lib/learning.js', module => console.log(`'${module}' Updated!`))
...
const start = async (tobz = new Client()) => {
        console.log('------------------------------------------------')
        lolcatjs.fromString(color(figlet.textSync('UTMs BOT', { horizontalLayout: 'full' })))
        console.log('------------------------------------------------')
        ...
        tobz.onAddedToGroup(async (chat) => {
            if(isWhite(chat.id)) return tobz.sendText(chat.id, 'Halo aku UTMs BOT, Ketik #help Untuk Melihat List Command Ku...')
            if(mtcState === false){
                const groups = await tobz.getAllGroups()
                // BOT group count less than
                if(groups.length > groupLimit){
                    await tobz.sendText(chat.id, 'Maaf, Batas group yang dapat UTMs BOT tampung sudah penuh').then(async () =>{
                    ...
                    if(chat.groupMetadata.participants.length < memberLimit){
                        ...
                        if(!chat.isReadOnly) tobz.sendText(chat.id, 'Halo aku UTMs BOT, Ketik #help Untuk Melihat List Command Ku...')
                    }
                }
            }else{
                await tobz.sendText(chat.id, 'UTMs BOT sedang maintenance, coba lain hari').then(async () => {
                    tobz.deleteChat(chat.id)
                    tobz.leaveGroup(chat.id)
                })
            }
